...
export interface ForceFieldOptions {
  ...
  /** Capture live HTML into the canvas. Set false to keep a GPU overlay on real DOM. */
  capture?: boolean;
...
export function createForceField(
  ...
  function syncCanvasSize() {
    ...
    const cssW = output.clientWidth || host?.clientWidth || window.innerWidth;
    const cssH = output.clientHeight || host?.clientHeight || window.innerHeight;
...
export function ForceField({
  ...
  const native = supported && !failed && initialOptions.capture !== false;
